...
At this point horizontal scaling on its own is not strictly required as it's possible to scale using hackage mirrors. But even in this case availability of the horizontal scaling allows additional functionality such as zero downtime rollouts, not available otherwise.
...
On the duration of the migration we plan to use existing TUF and keep on using `hackage-security`, unless it is proven to be impossible. Any changes or improvements in the framework are the point separate discussions.
...
The minimal viable product would be to rewrite packagedb and keep userdb in old hackage-server or using acid-state. But this solution would be incomplete not would work as a reasonable step.

We propose that MVP may be completed by 2 engineers in 3 months. Excluding the time that should be spent on monitoring of the phase completions. We propose 6 engineer*month as a high cap on the budget, but will try to deliver full solution: switch for both packagedb and userdb.
